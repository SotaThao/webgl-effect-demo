...
(function(){
  ...
  // 3. Multi-layer smoke with ripple effect
  const layers = [];
  const configs = [
    { scale:1.0, speed:0.05, weight:0.6, c1:[0.8,0.8,0.85], c2:[1,1,1] },
    { scale:1.3, speed:0.08, weight:0.3, c1:[0.6,0.6,0.7],  c2:[0.9,0.9,1] },
    { scale:1.6, speed:0.12, weight:0.1, c1:[0.4,0.5,0.6],  c2:[0.8,0.85,1] },
  ];
  ...
  configs.forEach(cfg => {
    const mat = new THREE.RawShaderMaterial({
      ...
      uniforms: {
        ...
        u_c1:    { value: new THREE.Vector3(...cfg.c1) },
        u_c2:    { value: new THREE.Vector3(...cfg.c2) },
      },
      vertexShader: `
      ...
      fragmentShader: `
        ...
        void main(){
          ...
          float ripple = sin(d*12.0 - u_time*6.0)*0.04;
          uv += normalize(uv - mpos)*ripple;

          // noise sampling
          vec2 p = uv*u_scale + u_time*u_speed;
          float n = fbm(p);
          float smoke = smoothstep(0.2,0.6,n);

          // color and alpha
          vec3 col = mix(u_c1, u_c2, smoke);
          float alpha = clamp(smoke * u_weight, 0.0, 1.0);
